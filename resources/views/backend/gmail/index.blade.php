...
@section('title', 'Gmail Accounts Management')

@section('content')
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Gmail Accounts Management</h1>
        <a href="{{ route('admin.gmail.create') }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            <h6 class="m-0 font-weight-bold text-primary">Gmail Account List</h6>
            <div>
                <div class="btn-group me-2">
                    <a href="{{ route('admin.gmail.index') }}" class="btn btn-sm {{ !request('trashed') ? 'btn-primary' : 'btn-outline-primary' }}">
                        Active
                    </a>
                    <a href="{{ route('admin.gmail.index', ['trashed' => 'true']) }}" class="btn btn-sm {{ request('trashed') === 'true' ? 'btn-danger' : 'btn-outline-danger' }}">
                        Trashed
                    </a>
                </div>
        ...
        <div class="collapse" id="filterCollapse">
            <div class="card-body border-bottom">
                <form action="{{ route('admin.gmail.index') }}" method="GET" class="row g-3">
                    <!-- Search -->
                    ...
                    <div class="col-md-4 mb-3 d-flex align-items-end">
                        <div class="btn-group">
                            ...
                            <a href="{{ route('admin.gmail.index', request('trashed') ? ['trashed' => request('trashed')] : []) }}" class="btn btn-secondary">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $account->trashed() ? 'table-danger' : '' }}">
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    @if($account->trashed())
                                        <form action="{{ route('admin.gmail.restore', $account->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-success" title="Restore">
                                                <i class="fas fa-trash-restore"></i>
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.gmail.force-delete', $account->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @else
                                        <a href="{{ route('admin.gmail.show', $account->id) }}" class="btn btn-sm btn-info" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.gmail.edit', $account->id) }}" class="btn btn-sm btn-warning" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.gmail.destroy', $account->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="10" class="text-center">No Gmail accounts found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
